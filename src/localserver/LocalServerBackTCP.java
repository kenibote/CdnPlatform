...
public class LocalServerBackTCP implements Runnable {
	/**
	 * 
	 * */
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				// 将该信息写入到公共空间中
				LocalServerPublicSetting.MonitorSocketInput = input;
				LocalServerPublicSetting.MonitorSocketWrite = write;
				...
				while (!socket.isClosed() && (getCommand = input.readLine()) != null) {
					...
					if (LocalServerPublicSetting.Key.equals(key)) {
						// 正确，则进入命令执行函数
						LocalServerControl.Router(getCommand);
					} else {
						// 否则鉴权错误，主动关闭socket
						result.clear();
				...
				LocalServerPublicSetting.MonitorSocketInput = null;
				LocalServerPublicSetting.MonitorSocketWrite = null;
