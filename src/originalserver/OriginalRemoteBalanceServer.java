...
public class OriginalRemoteBalanceServer implements Runnable{
	/**
	 * 该线程也应该是一直运行
	 * 
	 */
	private static Logger logger = LogManager.getLogger(OriginalRemoteBalanceServer.class.getName());

	// 服务器监听端口，默认8071
	private int server_port = OriginalServerPublicSetting.RemoteBalancePort;
	...
	private static void findServerForRemote(String ID) {
		...
		if (port != -1) {
			...
			result.put("IP", OriginalServerPublicSetting.OriginalServerIP);
